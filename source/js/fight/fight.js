...
function selectEnemy(enemyKey) {
    ...
    document.getElementById('battleArena').classList.add('battle-arena--active');
    document.getElementById('battleLogs').classList.add('battle-logs--active');
    
    updateUI();
    addLogEntry('Battle started against ' + gameState.enemy.name + '!');
    
    document.getElementById('roundInfo').textContent = 'Round 1 | Fighting ' + gameState.enemy.name;
}
...
function resetGame() {
    ...
    if (opponentSelector) opponentSelector.style.display = 'block';
    ...
    if (gameOverModal) gameOverModal.classList.remove('game-over-modal--active');
